...
// remediation, and how sure the review is.
...
// TestInlineCommentConfidenceRounding checks the percentage is rendered from the
// confidence value rather than invented.
func TestInlineCommentConfidenceRounding(t *testing.T) {
	tests := []struct {
		confidence float64
		want       string
	}{
	...
	for _, tt := range tests {
		...
		if body := NewRenderer().InlineComment(f); !strings.Contains(body, tt.want) {
			t.Errorf("confidence %v: comment does not contain %q", tt.confidence, tt.want)
		}
	}
}
...
func TestInlineCommentIsBounded(t *testing.T) {
	...
	for _, want := range []string{"COR-001", "HIGH", f.Title, "**Impact:**", "**Suggestion:**", "Confidence: 96%"} {
		if !strings.Contains(body, want) {
