...
type Context struct {
	...
	// Discussion is what humans have already said on this pull request: the
	// description, and the comments. It is untrusted text, and it is context about
	// intent rather than instruction.
...
// DiscussionContext is the human conversation attached to a pull request.
type DiscussionContext struct {
	Comments []Comment
}

// Count returns the number of comments carried.
func (d DiscussionContext) Count() int { return len(d.Comments) }

// Comment is one human remark, normalized independently of its transport.
type Comment struct {
	// Kind is "conversation" or "inline".
	Kind   string
	Author string
	Body   string
...
// WithDiscussion returns a snapshot with the pull request's human conversation
// attached.
//
// ARC's own comments are excluded. Its previous review is already accounted for by
// the publication history, and feeding its own words back in would let it treat
// its earlier opinion as independent evidence for the same opinion.
func WithDiscussion(ctx Context, comments []github.Comment) Context {
	if len(comments) == 0 {
		return ctx
	}

	ctx.Discussion.Comments = make([]Comment, 0, len(comments))
	for _, comment := range comments {
		if isARCComment(comment.Body) {
			continue
		}
		ctx.Discussion.Comments = append(ctx.Discussion.Comments, Comment{
...
// arcSignatures are the marks ARC leaves on what it publishes. Matching any of
// them means the text is this tool's own output.
var arcSignatures = []string{
	"arc-review:v1",
	"arc-findings:v1",
...
// isARCComment reports whether a comment is ARC's own writing.
func isARCComment(body string) bool {
	for _, signature := range arcSignatures {
